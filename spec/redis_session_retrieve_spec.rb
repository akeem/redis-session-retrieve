require 'mock_redis'
require File.expand_path("redis_session_retrieve", File.dirname(__FILE__) + "/../lib/redis-session-retrieve/")

describe RedisSessionRetrieve do

  let(:bootstrap_redis){
    mr = MockRedis.new
    mr.set("12345", Marshal.dump({:user_id => "the_one_handed_man"}))
    RedisSessionRetrieve.connection = mr
  }

  let(:existing_session){RedisSessionRetrieve.find_by_id('12345')}
  let(:missing_session){RedisSessionRetrieve.find_by_id('3333')}
